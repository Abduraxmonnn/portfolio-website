      ...
      <div id="section-content" class="hidden lg:block w-full h-full border-right">
        ...
        <div id="contact-sources" class="hidden lg:flex lg:flex-col my-2">
          <div v-for="(source, key) in config.dev.contacts.direct.sources" :key="key" class="flex items-center mb-2">
            ...
            <a v-if="source.startsWith('+998')" :href="'tel:' + source" class="font-fira_retina text-menu-text hover:text-white">{{source}}</a>
            <a v-else :href="'https://t.me/' + getValueAfterFirstElement(source)" target="_blank" class="font-fira_retina text-menu-text hover:text-white">{{source}}</a>
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    getValueAfterFirstElement(source) {
      if (!source) return "";
      return source.slice(1); // Or source.substring(1);
    },
  },
  mounted() {
    this.loading = false
